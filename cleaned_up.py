...
#    log_big_t = [[np.log(j) for j in i] for i in big_t_values]
#    log_small_t = [[np.log(j) for j in i] for i in small_t_values]
#    difference = [[(log_big_t[i][j] - log_small_t[i][j])\
#                   for j in range(len(x))] for i in range(len(log_big_t))]
#    for i in range(len(files) + 1):
...
#    draw
#    s1=mlab.triangular_mesh(X[:,0],X[:,1],X[:,2], T, scalars=vecs[:,0])
#    mlab.colorbar()
#    mlab.title('Dirichlet base eigenfunction')
#    mlab.show()